...
public:
  ...
  // Use the program
  void Use() const { glUseProgram(get_programID()); }

  // returns program ID
  GLuint get_programID() const { return program_id; }

  // used to set for example model matrix [from AbstractObj]
  inline void setMat4Uniform(const std::string& name,
                             const glm::mat4& mat) const {
    glUniformMatrix4fv(glGetUniformLocation(get_programID(), name.c_str()), 1,
                       GL_FALSE, &mat[0][0]);
  }

  // used to set any Vec4 parameter for shader
  inline void setVec4Uniform(const std::string& name,
                             const glm::vec4& vec) const {
    glUniform4fv(glGetUniformLocation(get_programID(), name.c_str()), 1,
                 &vec[0]);
  }

  // used to set any Vec3 parameter for shader
  inline void setVec3Uniform(const std::string& name,
                             const glm::vec3& vec) const {
    glUniform3fv(glGetUniformLocation(get_programID(), name.c_str()), 1,
                 &vec[0]);
  }

  // used to set any Vec2 parameter for shader
  inline void setVec2Uniform(const std::string& name,
                             const glm::vec2& vec) const {
    glUniform2fv(glGetUniformLocation(get_programID(), name.c_str()), 1,
                 &vec[0]);
  }

  // used to set any float parameter for shader
  inline void setFloatUniform(const std::string& name,
  ...
  inline void setUnsignedIntUniform(const std::string& name,
