...
// Light Manager's task is to manage scene lighting. It keeps all the lights,
// has one method for setting them all up at once, allows to dynamically add
// and/or remove them.
class LightManager {
public:
  // Currently lights in shaders are kept as an arrays of specified size
  // therefore LightManager has to keep track of amount of lights and if it is
  // close to the limit
  LightManager(int maxPointLightAmount, int maxSpotLightAmount);

  void setUpLight(const ShaderProgram& shaderProgram);

  // Directional light is only one therefore we do not need methods for adding
...
private:
  ...
  // Lights are kept here in vectors but shaders knows them by their index
  // therefore everytime these vectors are changed we need to update names of
  // light's uniforms as they contain indexes
  ...
  const unsigned maxPointLightAmount;
  const unsigned maxSpotLightAmount;
};
