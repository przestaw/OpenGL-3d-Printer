...
void SpotLight::setUp(const ShaderProgram& shaderProgram)
{
	...
	shaderProgram.setVec3Uniform(std::string(getUniformName() + ".direction"), getDirection());
	shaderProgram.setFloatUniform(std::string(getUniformName() + ".cutOff"), getCutOff());
	shaderProgram.setFloatUniform(std::string(getUniformName() + ".outerCutOff"), getOuterCutOff());
}
