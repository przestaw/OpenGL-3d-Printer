...
{
	shaderProgram.setVec3Uniform(directionalLight->getUniformName() + ".direction", 
								 directionalLight->getDirection());

	shaderProgram.setVec3Uniform(directionalLight->getUniformName() + ".ambientStrength",
								 directionalLight->getAmbientStrength());

	shaderProgram.setVec3Uniform(directionalLight->getUniformName() + ".diffuseStrength",
								 directionalLight->getDiffuseStrength());

	shaderProgram.setVec3Uniform(directionalLight->getUniformName() + ".specularStrength",
...
void LightManager::setUpPointLights(const ShaderProgram& shaderProgram)
{
	for (auto light : pointLightsVector) {
		shaderProgram.setVec3Uniform(light->getUniformName() + ".position", light->getPosition());
		shaderProgram.setVec3Uniform(light->getUniformName() + ".ambientStrength", light->getAmbientStrength());
		shaderProgram.setVec3Uniform(light->getUniformName() + ".diffuseStrength", light->getDiffuseStrength());
		shaderProgram.setVec3Uniform(light->getUniformName() + ".specularStrength", light->getSpecularStrength());
		shaderProgram.setFloatUniform(light->getUniformName() + ".constantParameter", light->getConstantParameter());
...
// How could I rewrite it so it won't duplicate as much code but still be readable?
// Or it is okay the way it is?
void LightManager::setUpSpotLights(const ShaderProgram& shaderProgram)
{
	for (auto light : spotLightsVector) {
		shaderProgram.setVec3Uniform(light->getUniformName() + ".position", light->getPosition());
		shaderProgram.setVec3Uniform(light->getUniformName() + ".direction", light->getDirection());
		shaderProgram.setVec3Uniform(light->getUniformName() + ".ambientStrength", light->getAmbientStrength());
		shaderProgram.setVec3Uniform(light->getUniformName() + ".diffuseStrength", light->getDiffuseStrength());
		shaderProgram.setVec3Uniform(light->getUniformName() + ".specularStrength", light->getSpecularStrength());
		shaderProgram.setFloatUniform(light->getUniformName() + ".constantParameter", light->getConstantParameter());
