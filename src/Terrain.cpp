...
Terrain::Terrain(GLfloat width, GLfloat lenght, GLuint subdivisions,
                 GLfloat maxHeight, bool textureMappingByPrzemek) {
  /* Generate vertices */
  ...
  // Generate first vertex, with completely random height from the range [0,
  // maxHeight). Height of every next vertex will depend on the height of the
  // previous one.
  ...
  for (GLuint row = 0; row <= subdivisions; ++row) {
    for (GLuint col = 0; col <= subdivisions; ++col) {
      ...
      position = glm::vec3(col * width / subdivisions - width / 2.0f, height,
                           ((subdivisions - row) * lenght) / subdivisions
                               - (lenght / 2.0f));
      texCoord = textureMappingByPrzemek
                     ? glm::vec2(static_cast<GLfloat>(col % 2),
                                 static_cast<GLfloat>(row % 2))
