...
void PointLight::setUp(const ShaderProgram& shaderProgram) {
  shaderProgram.setVec3Uniform(std::string(getUniformName() + ".position"),
                               getPosition());
  ...
  shaderProgram.setVec3Uniform(
      ...
      getSpecularStrength());
  shaderProgram.setFloatUniform(
      std::string(getUniformName() + ".constantParameter"),
      getConstantParameter());
  shaderProgram.setFloatUniform(
      std::string(getUniformName() + ".linearParameter"), getLinearParameter());
  shaderProgram.setFloatUniform(
      std::string(getUniformName() + ".quadraticParameter"),
      getQuadraticParameter());
}
...
void PointLight::setLightRange(const LightRange range) {
  ...
  std::tie(constantParameter, linearParameter, quadraticParameter) =
      rangeMap.find(range)->second;
...
GLfloat PointLight::getConstantParameter() { return constantParameter; }

void PointLight::setLinearParameter(const GLfloat parameter) {
  linearParameter = parameter;
}

GLfloat PointLight::getLinearParameter() { return linearParameter; }

void PointLight::setQuadraticParameter(const GLfloat parameter) {
  quadraticParameter = parameter;
}

GLfloat PointLight::getQuadraticParameter() { return quadraticParameter; }
