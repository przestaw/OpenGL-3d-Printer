...
Printer::Printer(GLfloat scale_c, unsigned maxPos_a)
    : scale(scale_c), maxPos(maxPos_a), posX(maxPos_a / 2), posY(maxPos_a / 2),
      posZ(maxPos_a / 2), step(0.8 * scale / maxPos), printerArm(scale),
      printerExtruder(scale), printerBase(scale), printerTop(scale) {

  printerArm.translate(glm::vec3(
      0.0, (0.13 + (static_cast<GLfloat>(posZ) / maxPos)) * scale, 0.0));
  printerExtruder.translate(
      glm::vec3(0.0, (0.13 + (static_cast<GLfloat>(posZ) / maxPos)) * scale,
                0.035 * scale));
}
...
void Printer::spawnIceCream(glm::vec3 color) {
  ...
  temp.translate(getNeadle()
                 + glm::vec3(0.0, -scale * (ratio * 2.5 * 1.17), 0.0));
...
inline glm::vec3 Printer::getNeadle() {
  return glm::vec3((-0.05 * scale) - (static_cast<double>(posX) * step)
                       + 0.4 * scale,
